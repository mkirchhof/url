...
def load_pretrained(
    ...
    use_spec_norm = hasattr(model, "use_spec_norm") and model.use_spec_norm

    class_name = model.__class__.__name__
    if class_name.endswith("HETXL") or (
        class_name.endswith("SNGP")
        and not use_spec_norm):
        strict = False
    elif class_name.endswith("SNGP") and use_spec_norm:
        model_state_dict = model.state_dict()
        for key in list(state_dict.keys()):
            if key not in model_state_dict:
                new_key = key.replace(".weight", ".layer.weight")
