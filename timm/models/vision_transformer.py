...
class VisionTransformerSNGP(nn.Module):
    """Vision Transformer with (SN)GP
    ...
    def __init__(
        ...
        if gp_output_imagenet_initializer:
            self.gp_output_initializer = random_normal(stddev=0.01)
        ...
        self.head = RandomFeatureGaussianProcess(
            ...
            kernel_initializer=self.gp_output_initializer,
        )
    ...
    def reset_classifier(self, num_classes: int, global_pool=None):
        ...
        self.head = RandomFeatureGaussianProcess(
            ...
            num_inducing=self.gp_hidden_dim,
...
class VisionTransformerDropout(VisionTransformer):
    def __init__(
        self,
        img_size=224,
        patch_size=16,
        ...
        pre_norm=False,
        fc_norm=None,
        drop_rate=0.1,  # Changed
        attn_drop_rate=0.1,  # Changed
        drop_path_rate=0.0,
        weight_init="",
        embed_layer=PatchEmbed,
        norm_layer=None,
...
class VisionTransformerHETXL(VisionTransformer):
    """Vision Transformer w/ HET-XL
...
def _create_vision_transformer_dropout(variant, pretrained=False, **kwargs):
    if kwargs.get("features_only", None):
        raise RuntimeError(
            "features_only not implemented for Vision Transformer models."
        )
...
def _create_vision_transformer_hetxl(variant, pretrained=False, **kwargs):
    if kwargs.get("features_only", None):
        raise RuntimeError(
            "features_only not implemented for Vision Transformer models."
        )
...
def _create_vision_transformer_sngp(variant, pretrained=False, **kwargs):
    if kwargs.get("features_only", None):
        raise RuntimeError(
            "features_only not implemented for Vision Transformer models."
        )
...
@register_model
def vit_medium_patch16_gap_256dropout(pretrained=False, **kwargs):
    """ViT-Medium (ViT-M/16) w/o class token, w/ avg-pool @ 256x256 w/ dropout"""
    model_kwargs = dict(
        patch_size=16,
        embed_dim=512,
        depth=12,
    ...
    model = _create_vision_transformer_dropout(
        "vit_medium_patch16_gap_256",
        pretrained=pretrained,
        **dict(model_kwargs, **kwargs),
    )
...
@register_model
def vit_medium_patch16_gap_256hetxl(pretrained=False, **kwargs):
    """ViT-Medium (ViT-M/16) w/o class token, w/ avg-pool @ 256x256 w/ HET-XL"""
    model_kwargs = dict(
        patch_size=16,
        embed_dim=512,
        depth=12,
    ...
    model = _create_vision_transformer_hetxl(
        "vit_medium_patch16_gap_256",
        pretrained=pretrained,
        **dict(model_kwargs, **kwargs),
    )
...
@register_model
def vit_medium_patch16_gap_256sngp(pretrained=False, **kwargs):
    """ViT-Medium (ViT-M/16) w/o class token, w/ avg-pool @ 256x256 w/ HET-XL"""
    model_kwargs = dict(
        patch_size=16,
        embed_dim=512,
    ...
    model = _create_vision_transformer_sngp(
        "vit_medium_patch16_gap_256",
        pretrained=pretrained,
        **dict(model_kwargs, **kwargs),
    )
