...
from timm.layers import DropBlock2d, DropPath, AvgPool2dSame, BlurPool2d, GroupNorm, create_attn, get_attn, \
    get_act_layer, get_norm_layer, create_classifier, make_conv2d_layer, \
    MonteCarloDropout, HETXLHead
...
class BasicBlockDropout(nn.Module):
    ...
    def __init__(
        ...
        assert cardinality == 1, 'BasicBlockDropout only supports cardinality of 1'
        assert base_width == 64, 'BasicBlockDropout does not support changing base width'
        first_planes = planes // reduce_first
        outplanes = planes * self.expansion
        first_dilation = first_dilation or dilation
        use_aa = aa_layer is not None and (stride == 2 or first_dilation != dilation)
    ...
    def forward(self, x):
        ...
        x = self.drop_block(x)
        x = self.act1(x)
        x = F.dropout(x, p=float(self.drop_rate), training=True)
        x = self.aa(x)
...
class BottleneckSNGP(nn.Module):
    expansion = 4
    ...
    def __init__(
        ...
        assert first_dilation == 1  # TODO: remove
...
def make_blocks_sngp(
    ...
    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        ...
        for block_idx in range(num_blocks):
            ...
            block_dpr = drop_path_rate * net_block_idx / (net_num_blocks - 1)  # stochastic depth linear decay rule
            blocks.append(block_fn(
                inplanes, planes, conv_layer, dropout_layer, stride, downsample, first_dilation=prev_dilation,
...
class ResNetDropout(nn.Module):
    """ResNet / ResNeXt / SE-ResNeXt / SE-Net w/ Dropout
    ...
    def __init__(
        ...
        """
        Args:
            block (nn.Module): class for the residual block. Options are BasicBlockDropout, BottleneckDropout.
            layers (List[int]) : number of layers in each block
            num_classes (int): number of classification classes (default 1000)
            in_chans (int): number of input (color) channels. (default 3)
            output_stride (int): output stride of the network, 32, 16, or 8. (default 32)
    ...
    @staticmethod
    def from_pretrained(model_name: str, load_weights=True, **kwargs) -> 'ResNet':
        entry_fn = model_entrypoint(model_name, 'resnet')  # TODO
        return entry_fn(pretrained=not load_weights, **kwargs)

    @torch.jit.ignore
    def init_weights(self, zero_init_last=True):
        for n, m in self.named_modules():
            if isinstance(m, nn.Conv2d):  # TODO: possibly buggy
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
        if zero_init_last:
            for m in self.modules():
                if hasattr(m, 'zero_init_last'):
...
class ResNetHETXL(ResNet):
    def __init__(
        ...
        self.global_pool, fc = create_classifier(
    ...
    def forward_head(self, x, pre_logits: bool = False):
        features = self.global_pool(x)  # [B, D]
        if self.drop_rate:
            features = F.dropout(features, p=float(self.drop_rate), training=self.training)
...
class ResNetSNGP(nn.Module):
    """ResNet / ResNeXt / SE-ResNeXt / SE-Net with SNGP.
    ...
    def __init__(
        ...
        """
        Args:
            block (nn.Module): class for the residual block. Options are BasicBlock, Bottleneck, BottleneckSNGP.
            layers (List[int]) : number of layers in each block
            num_classes (int): number of classification classes (default 1000)
            in_chans (int): number of input (color) channels. (default 3)
            output_stride (int): output stride of the network, 32, 16, or 8. (default 32)
        ...
        # Feature Blocks
        channels = [64, 128, 256, 512]
        stage_modules, stage_feature_info = make_blocks_sngp(
            block,
            channels,
            layers,
            inplanes,
        ...
        self.global_pool, self.fc = create_classifier(
            self.num_features, self.num_classes, pool_type=global_pool, use_SNGP=True,
    ...
    def reset_classifier(self, num_classes, global_pool='avg'):
        ...
        self.global_pool, self.fc = create_classifier(
            self.num_features, self.num_classes, pool_type=global_pool, use_SNGP=True,
            gp_hidden_dim=self.gp_hidden_dim,
            gp_scale=self.gp_scale,
            gp_bias=self.gp_bias,
            gp_input_normalization=self.gp_input_normalization,
            gp_random_feature_type=self.gp_random_feature_type,
            gp_cov_discount_factor=self.gp_cov_discount_factor,
            gp_cov_ridge_penalty=self.gp_cov_ridge_penalty,
            gp_output_imagenet_initializer=self.gp_output_imagenet_initializer,
        )
...
def _create_resnet_dropout(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(ResNetDropout, variant, pretrained, **kwargs)

def _create_resnet_hetxl(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(ResNetHETXL, variant, pretrained, **kwargs)

def _create_resnet_sngp(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(ResNetSNGP, variant, pretrained, **kwargs)
...
@register_model
def resnet50dropout(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model with dropout.
    """
    model_args = dict(block=BottleneckDropout, layers=[3, 4, 6, 3], **kwargs)
    return _create_resnet_dropout('resnet50', pretrained, **dict(model_args, **kwargs))


@register_model
def resnet50hetxl(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model with HET-XL.
    """
    model_args = dict(block=BottleneckDropout, layers=[3, 4, 6, 3], **kwargs)
    return _create_resnet_hetxl('resnet50', pretrained, **dict(model_args, **kwargs))

@register_model
def resnet50sngp(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model with SNGP.
    """
    model_args = dict(block=BottleneckSNGP, layers=[3, 4, 6, 3], **kwargs)
    return _create_resnet_sngp('resnet50', pretrained, **dict(model_args, **kwargs))
