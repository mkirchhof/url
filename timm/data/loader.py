...
def fast_collate(batch):
    ...
    if isinstance(batch[0][0], tuple):
        ...
        for i in range(batch_size):
            tensor[i].copy_(batch[i][0])
        return tensor, targets
    elif isinstance(batch[0][0], list) and isinstance(batch[0][0][0], np.ndarray):
        # We have contrastive positive pairs
        # The contrastive dataloader gives us pairs [[pos0, pos0], [pos1, pos1], ...]
        # flatten them into the format [pos0, pos0, pos1, pos1, ...] to make it compatible to normal training and eval
        # The contrastive losses will take care of casting them back to [[pos0, pos0], [pos1, pos1], ...]
        n_positive = len(batch[0][0])
        targets = torch.from_numpy(np.array([[b[1] for _ in range(n_positive)] for b in batch])).flatten()
        assert len(targets) == batch_size * n_positive
        tensor = torch.zeros((batch_size * n_positive, *batch[0][0][0].shape), dtype=torch.uint8)
        for i in range(batch_size):
            for p in range(n_positive):
                tensor[i * n_positive + p] += batch[i][0][p]
        return tensor, targets
    else:
        assert False
